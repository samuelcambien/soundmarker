...
Player.Spectrogram = {
    ...
    init: function (params) {
        ...
        if (!this.container) {
            throw Error('No container for Player spectrogram');
        }

        this.width = drawer.width;
        this.pixelRatio = this.params.pixelRatio || player.params.pixelRatio;
        this.fftSamples = this.params.fftSamples || player.params.fftSamples || 512;
        ...
        player.on('redraw', this.render.bind(this));
    ...
    createCanvas: function () {
        ...
        this.player.drawer.style(canvas, {
            position: 'absolute',
            zIndex: 4
        });
    },
    ...
    drawSpectrogram: function(frequenciesData, my) {
        ...
        var length = my.player.backend.getDuration();
    ...
    getFrequencies: function(callback) {
        ...
        var buffer = this.buffer = this.player.backend.buffer;
        ...
        var fft = new Player.FFT(fftSamples, sampleRate, this.windowFunc, this.alpha);
    ...
    loadFrequenciesData: function (url) {
        ...
        var ajax = Player.util.ajax({ url: url });
    ...
    loadLabels: function (bgFill, fontSizeFreq, fontSizeUnit, fontType, textColorFreq, textColorUnit, textAlign, container) {
        ...
        var freqStart = 0;
        var step = ((this.player.backend.ac.sampleRate / 2) - freqStart) / labelIndex;
 ...
 */
Player.FFT = function(bufferSize, sampleRate, windowFunc, alpha) {
...
Player.util.extend(Player.Spectrogram, Player.Observer, Player.FFT);
