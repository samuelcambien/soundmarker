...
Player.Regions = {
    init: function (player) {
        this.player = player;
        this.wrapper = this.player.drawer.wrapper;
    ...
    /* Add a region. */
    add: function (params) {
        var region = Object.create(Player.Region);
        region.init(params, this.player);
    ...
    enableDragSelection: function (params) {
        ...
        var eventDown = function (e) {
            ...
            drag = true;
            start = my.player.drawer.handleEvent(e, true);
            region = null;
        };
        ...
        var eventMove = function (e) {
            ...
            var duration = my.player.getDuration();
            var end = my.player.drawer.handleEvent(e);
            region.update({
...
Player.util.extend(Player.Regions, Player.Observer);

Player.Region = {
    /* Helper function to assign CSS styles. */
    style: Player.Drawer.style,

    init: function (params, player) {
        this.player = player;
        this.wrapper = player.drawer.wrapper;

        this.id = params.id == null ? Player.util.getId() : params.id;
        this.start = Number(params.start) || 0;
        this.end = params.end == null ?
            // small marker-like region
            this.start + (4 / this.wrapper.scrollWidth) * this.player.getDuration() :
            Number(params.end);
        ...
        this.player.on('zoom', this.onZoom);

        this.player.fireEvent('region-created', this);
    ...
    update: function (params) {
        ...
        this.player.fireEvent('region-updated', this);
    },
    ...
    remove: function () {
        if (this.element) {
            ...
            this.element = null;
            this.player.un('zoom', this.onZoom);
            this.fireEvent('remove');
            this.player.fireEvent('region-removed', this);
        }
    },
    ...
    play: function () {
        ...
        this.fireEvent('play');
        this.player.fireEvent('region-play', this);
    },
    ...
    getWidth: function () {
        return this.player.drawer.width / this.player.params.pixelRatio;
    },
    ...
    bindInOut: function () {
        ...
        var onProcess = function (time) {
            if (!my.firedOut && my.firedIn && (my.start >= Math.round(time * 100) / 100 || my.end <= Math.round(time * 100) / 100)) {
                ...
                my.firedIn = false;
                my.fireEvent('out');
                my.player.fireEvent('region-out', my);
            }
            if (!my.firedIn && my.start <= time && my.end > time) {
                my.firedIn = true;
                my.firedOut = false;
                my.fireEvent('in');
                my.player.fireEvent('region-in', my);
            }
        };

        this.player.backend.on('audioprocess', onProcess);

        this.on('remove', function () {
            my.player.backend.un('audioprocess', onProcess);
        });

        /* Loop playback. */
        this.on('out', function () {
            if (my.loop) {
                my.player.play(my.start);
            }
        });
    },
    ...
    bindEvents: function () {
        ...
        this.element.addEventListener('mouseenter', function (e) {
            ...
            my.player.fireEvent('region-mouseenter', my, e);
        });

        this.element.addEventListener('mouseleave', function (e) {
            my.fireEvent('mouseleave', e);
            my.player.fireEvent('region-mouseleave', my, e);
        });

        this.element.addEventListener('click', function (e) {
            e.preventDefault();
            my.fireEvent('click', e);
            my.player.fireEvent('region-click', my, e);
        });
        ...
        this.element.addEventListener('dblclick', function (e) {
            ...
            my.player.fireEvent('region-dblclick', my, e);
        });
        ...
        (this.drag || this.resize) && (function () {
            ...
            var onMove = function (e) {
                ...
                if (drag || resize) {
                    var time = my.player.drawer.handleEvent(e) * duration;
    ...
    onDrag: function (delta) {
        var maxEnd = this.player.getDuration();
...
Player.util.extend(Player.Region, Player.Observer);


/* Augment Player with region methods. */
Player.initRegions = function () {
    if (!this.regions) {
        this.regions = Object.create(Player.Regions);
        this.regions.init(this);
    }
};

Player.addRegion = function (options) {
    this.initRegions();
    return this.regions.add(options);
};

Player.clearRegions = function () {
    this.regions && this.regions.clear();
};

Player.enableDragSelection = function (options) {
    this.initRegions();
    this.regions.enableDragSelection(options);
};

Player.disableDragSelection = function () {
